---
title: Java题库-下
date: 2021-08-25 13:37:00 +0800
categories: [面试, Java]
tags: [Java]
pin: false
---

这部分主要是开源Java EE框架方面的内容，包括Hibernate、MyBatis、Spring、Spring MVC等，由于Struts 2已经是明日黄花，在这里就不讨论Struts 2的面试题，如果需要了解相关内容，可以参考文章[《Java面试题集（86-115）》](http://blog.csdn.net/jackfrued/article/details/17596171){:target="_blank"} 。此外，这篇文章还对企业应用架构、大型网站架构和应用服务器优化等内容进行了简单的探讨，这些内容相信对面试会很有帮助。
...
持久层框架有：  
\- [Hibernate](http://hibernate.org){:target="_blank"}  
\- [MyBatis](http://blog.mybatis.org){:target="_blank"}  
\- [TopLink](http://www.oracle.com/technetwork/cn/middleware/toplink/overview/index.html){:target="_blank"}  
\- [Guzz](https://code.google.com/p/guzz/){:target="_blank"}  
\- [jOOQ](http://www.jooq.org){:target="_blank"}  
\- [Spring Data](http://projects.spring.io/spring-data/){:target="_blank"}  
\- [ActiveJDBC](https://code.google.com/p/activejdbc/){:target="_blank"}
...
Session会延迟获取数据库连接（也就是在需要的时候才会获取）。  
...
至于lock()方法和update()方法的区别：  
- update()方法是把一个已经更改过的脱管状态的对象变成持久状态；  
...
> **说明：**关于N+1查询问题，可以参考CSDN上的一篇文章  [《什么是N+1查询》](http://blog.csdn.net/xtayhicbladwin/article/details/4739852){:target="_blank"}  
...
悲观锁，顾名思义悲观的认为在数据处理过程中极有可能存在修改数据的并发事务（包括本系统的其他事务或来自外部系统的事务），于是将处理的数据设置为锁定状态。悲观锁必须依赖数据库本身的锁机制才能真正保证数据访问的排他性，关于数据库的锁机制和事务隔离级别在[《Java面试题大全（上）》](https://www.jiazhenjiang.com/posts/Java%E9%9D%A2%E8%AF%95%E9%A2%98-%E4%B8%8A/){:target="_blank"} 中已经讨论过了。  
...
![这里写图片描述](https://cdn.jsdelivr.net/gh/3wsea/blog-images@master/commons/20210825/0825-1-1.jpg)
...
> **提示：**关于这个问题，在[Hibernate的官方文档](http://docs.jboss.org/hibernate/orm/4.3/devguide/en-US/html_single/#d5e874){:target="_blank"}中有更为详细的解读。
...
![这里写图片描述](https://cdn.jsdelivr.net/gh/3wsea/blog-images@master/commons/20210825/0825-1-5.jpg)  
...
\- 频繁修改的数据；  
\- 数据不一致与脏读；  
\- 缓存雪崩（可以采用分布式缓存服务器集群加以解决，[memcached](http://memcached.org){:target="_blank"}是广泛采用的解决方案）；  
\- 缓存预热；  
\- 缓存穿透（恶意持续请求不存在的数据）。  
...
![这里写图片描述](https://cdn.jsdelivr.net/gh/3wsea/blog-images@master/commons/20210825/0825-1-2.jpg)

> **补充：**防火墙的架设是Web安全的重要保障，[ModSecurity](http://www.modsecurity.org/){:target="_blank"}是开源的Web防火墙中的佼佼者。企业级防火墙的架设应当有两级防火墙，Web服务器和部分应用服务器可以架设在两级防火墙之间的DMZ，而数据和资源服务器应当架设在第二级防火墙之后。
...
贫血模型  
![这里写图片描述](https://cdn.jsdelivr.net/gh/3wsea/blog-images@master/commons/20210825/0825-1-3.jpg)

充血模型  
![这里写图片描述](https://cdn.jsdelivr.net/gh/3wsea/blog-images@master/commons/20210825/0825-1-4.jpg)
...
然而，事务脚本模式的缺点也是很多的，随着领域逻辑复杂性的增加，系统的复杂性将迅速增加，程序结构将变得极度混乱。开源中国社区上有一篇很好的译文[《贫血领域模型是如何导致糟糕的软件产生》](http://www.oschina.net/translate/how-anaemic-domain-models-cause-bad-software){:target="_blank"}对这个问题做了比较细致的阐述。
...
TDD可以在多个层级上应用，包括单元测试（测试一个类中的代码）、集成测试（测试类之间的交互）、系统测试（测试运行的系统）和系统集成测试（测试运行的系统包括使用的第三方组件）。TDD的实施步骤是：红（失败测试）- 绿（通过测试） - 重构。关于实施TDD的详细步骤请参考另一篇文章[《测试驱动开发之初窥门径》](http://blog.csdn.net/jackfrued/article/details/44433249){:target="_blank"}。  
